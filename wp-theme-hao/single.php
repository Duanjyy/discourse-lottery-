...
<th:block th:fragment="head">
    <th:block th:replace="~{modules/common/open-graph :: open-graph(_title = <?php the_title(); ?>,
                _permalink = <?php the_permalink(); ?>,
...
<th:block th:fragment="content">
    <div class="post" id="body-wrap">
        ...
        <header class="post-bg" id="page-header">
            <nav th:replace="~{modules/nav :: nav(title = <?php the_title(); ?>)}"></nav>
            ...
            <div id="post-info">
                    ...
                    <div class="meta-firstline">
                        ...
                        <th:block th:with="copyrightUrl =${#annotations.get(post, 'copyrightUrl')}"
                            ...
                            <a class="post-meta-original"
                               ...
                               <?php /* if(${#strings.equals(#annotations.getOrDefault(post, 'copyrightType','original'),'original')}) */ ?>
                            ...
                            <a class="post-meta-original"
                               ...
                               <?php /* if(${#strings.equals(#annotations.getOrDefault(post, 'copyrightType','original'),'reprint')}) */ ?>
                               th:href="@{${not #strings.isEmpty(copyrightUrl) ? copyrightUrl :
                ...
                <h1 class="post-title" th:text="<?php the_title(); ?>"></h1>
                <div id="post-meta">
                    <div class="meta-secondline">
                        ...
                        <span class="post-meta-date" <?php /* if(${theme.config.post.update_time}) */ ?>>
                        ...
                        <span th:with="syncStatus=${#strings.isEmpty(#annotations.get(post, 'sync_status')) ? '未同步': #annotations.get(post, 'sync_status')}"
                            ...
                            <th:block <?php /* if(${pluginFinder.available('plugin-platforms-sync') && (syncStatus == '同步更新文章成功' || syncStatus == '同步上传文章成功')}) */ ?>>
                        ...
                        <a <?php /* if(${post.spec.allowComment && #strings.contains('Twikoo,Artalk,commentWidget',theme.config.comments.use)}) */ ?>
                           ...
                           href="#post-comment">
                            ...
                            <span <?php /* if(${#strings.equals(theme.config.comments.use, 'commentWidget')}) */ ?>
                                  id="comment-count" th:text="${post.stats.comment}">
                                <i class="haofont hao-icon-spinner fa-spin"></i>
                            </span>
                            <span <?php /* if(${#strings.equals(theme.config.comments.use, 'Twikoo')}) */ ?>
                                  id="twikoo-count">
                                <i class="haofont hao-icon-spinner fa-spin"></i>
                            </span>
                            <span <?php /* if(${#strings.equals(theme.config.comments.use, 'Artalk')}) */ ?>
                                  id="ArtalkCount">
                                <i class="haofont hao-icon-spinner fa-spin"></i>
                            </span>
                        </a>
                        <th:block th:with="contributorLogin = ${contributorFinder.getContributor(#authentication.name)}">
                            <a class="post-meta-editor" sec:authorize="isAuthenticated()" data-flag-title="编辑文章"
                               title="编辑文章"
                               <?php /* if(${contributorLogin.name} == ${post.spec.owner}) */ ?>
            ...
            <div id="post">
                ...
                <div class="post-ai" <?php /* if(${not #strings.isEmpty(#annotations.get(post, 'ai')) ?
                    ...
                    <div class="ai-title"><i class="haofont hao-icon-bilibili"></i>
                        ...
                        <div <?php /* if(${theme.config.post.aiDescription.switchBtn}) */ ?> id="ai-Toggle">切换</div>
                        <i class="haofont hao-icon-arrow-rotate-right"></i>
                        <div <?php /* if(${#strings.equals(theme.config.post.aiDescription.mode, 'local')}) */ ?> class="ai-tag" id="ai-tag">[[${theme.config.post.aiDescription.gptName}]] GPT</div>
                        <div <?php /* if(${#strings.equals(theme.config.post.aiDescription.mode, 'tianli')}) */ ?> class="ai-tag" id="ai-tag">Tianli GPT</div>
                    </div>
                ...
                <article  th:class="'post-content  '+${ theme.config.code.enable_line || pluginFinder.available('PluginPrismJS') ? 'line-numbers' : ''}" id="article-container">
                    <th:block <?php /* if(${theme.config.post.passage_tips.enable}) */ ?>
                              th:with="days=${(new java.util.Date().getTime()-post.status.lastModifyTime.toEpochMilli())/86400000}">
                        <div class="note simple  warning"  <?php /* if(${days > #conversions.convert(theme.config.post.passage_tips.day, 'java.lang.Integer')}) */ ?>>
                            <p>
                ...
                <nav class="pagination-post needEndHide" id="pagination"
                    ...
                    <div <?php /* if(${postCursor.hasPrevious()}) */ ?> th:class="${postCursor.hasNext()==false} ? 'prev-post pull-full ' : 'prev-post pull-left'">
                    ...
                    <div <?php /* if(${postCursor.hasNext()}) */ ?> th:class="${postCursor.hasPrevious()==false} ? 'next-post pull-full ':'next-post pull-right'">
